...
jQuery.validator.addMethod("nick",function(value, element) {
	return this.optional(element) || checkNick(value);
},"该昵称已注册！");
...
jQuery.validator.addMethod("link",function(value, element) {
	return this.optional(element) || checkLink(value);
},"该广告活动不存在！");

//校验商品名称
jQuery.validator.addMethod("item",function(value, element) {
	return this.optional(element) || checkItem(value);
},"该商品名称已存在！");
...
function checkNick(nick){
	...
	$.ajax({  
        ...
        success : function(msg) { 
        	if(msg.val == 0){  // 昵称未重复
        		$("#checkNick").val(true);
        	}else if(msg.val == 1){  // 昵称重复
        		$("#checkNick").val(false);
        	}else{  // 修改时，只触发，但没做任何操作
        		$("#checkNick").val(true);
        	}
        }  
    });
	var result = $("#checkNick").val();
	if(result == 'true'){
		result = true;
	}else{
		result = false;
